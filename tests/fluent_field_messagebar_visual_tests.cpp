...
int main(int argc, char** argv)
{
    try {
        ...
        wui::FieldNode normal("Task title"); normal.setHint("Keep it short and actionable"); normal.setRequired(true); normal.setControl(std::make_unique<wui::TextFieldNode>("Add a title")); draw(normal, paint, {28,74,330,96});
        wui::FieldNode invalid("Project name"); invalid.setOrientation(wui::FieldOrientation::Horizontal); invalid.setValidationState(wui::FieldValidationState::Error); invalid.setValidationMessage("This field is required"); invalid.setControl(std::make_unique<wui::TextFieldNode>("Required")); draw(invalid, paint, {400,74,330,88});
        wui::MessageBarNode info("Your changes are saved automatically."); info.setTitle("Tip"); info.setDismissible(true); draw(info, paint, {28,210,704,64});
        wui::MessageBarNode warning("This action cannot be undone. Review the selected tasks before continuing."); warning.setTitle("Review changes"); warning.setIntent(wui::MessageBarIntent::Warning); warning.setMultiline(true); warning.addAction({"Undo", {}}); warning.setDismissible(true); draw(warning, paint, {28,302,704,112});
