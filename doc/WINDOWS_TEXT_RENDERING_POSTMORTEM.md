...
后续在字体清晰度修复后又发现了独立的控件 baseline 下沉问题。它不属于
ClearType 或 DPR 故障；同一批完成的 Fluent 字号阶梯、字体回退、字重缓存隔离
和四档 DPR 验证也记录在
[Fluent control text alignment postmortem](whatsui/FLUENT_TEXT_BASELINE_POSTMORTEM.md)。
> 最后更新：2026-07-19

图片说明：`doc/images` 中的显示器近摄照片由用户在排查中提供，应用截图由对应构建直接捕获。显示器照片包含相机曝光、摩尔纹、前景/背景反相等变量，只用于呈现现象；真正的回归结论以应用直接 framebuffer capture 和自动化像素测试为准。旧原始基线没有可重建的唯一 commit/build id，因此本文不为历史图片虚构版本标识。
...
- 可复现构建目录为 `build-native-text-compare`；历史坏构建图片仅作为现象证据。
...
需要特别区分：文字“锐利”不等于文字“排版正确”。ClearType、DPR 和字重
解决的是字形栅格质量；ascent/descent baseline 与应用行容器解决的是控件内
对齐。两条链路都必须通过，组件才达到可交付视觉质量。
