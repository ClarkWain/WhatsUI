...
| Text shaping/fallback | On Windows the default renderer uses DirectWrite/native glyph rasterization with HarfBuzz shaping, preserving ClearType for small UI text. FreeType rasterization remains an explicit opt-in; other platforms keep the portable FreeType default. | Font availability and fallback differ by Windows installation. CJK, emoji, bidi, and the cache have automated coverage, but application-specific typography still needs visual review. |
...
editable Value, focus, Selection, and SelectionItem actions are synchronously
marshalled to the UI thread and verified against refreshed native state. Native
property, focus, bounds, and structure events are verified from an MTA UIA
subscriber. Rich text ranges, high-contrast policy, and the
