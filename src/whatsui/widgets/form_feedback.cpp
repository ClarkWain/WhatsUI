...
FieldNode::FieldNode(std::string v):label_(std::move(v)){}
FieldNode& FieldNode::label(std::string v){setLabel(std::move(v));return *this;} void FieldNode::setLabel(std::string v){if(label_!=v){label_=std::move(v);syncControlSemantics();markDirty(DirtyFlag::Layout);}} const std::string& FieldNode::label()const noexcept{return label_;}
FieldNode& FieldNode::hint(std::string v){setHint(std::move(v));return *this;} void FieldNode::setHint(std::string v){if(hint_!=v){hint_=std::move(v);markDirty(DirtyFlag::Layout);}} const std::string& FieldNode::hint()const noexcept{return hint_;}
FieldNode& FieldNode::validationMessage(std::string v){setValidationMessage(std::move(v));return *this;} void FieldNode::setValidationMessage(std::string v){if(validationMessage_!=v){validationMessage_=std::move(v);syncControlSemantics();markDirty(DirtyFlag::Layout);}} const std::string& FieldNode::validationMessage()const noexcept{return validationMessage_;}
FieldNode& FieldNode::validationState(FieldValidationState v)noexcept{setValidationState(v);return *this;} void FieldNode::setValidationState(FieldValidationState v)noexcept{if(validationState_!=v){validationState_=v;syncControlSemantics();markDirty(DirtyFlag::Paint);}} FieldValidationState FieldNode::validationState()const noexcept{return validationState_;}
FieldNode& FieldNode::required(bool v)noexcept{setRequired(v);return *this;} void FieldNode::setRequired(bool v)noexcept{if(required_!=v){required_=v;markDirty(DirtyFlag::Paint);}} bool FieldNode::isRequired()const noexcept{return required_;}
FieldNode& FieldNode::orientation(FieldOrientation v)noexcept{setOrientation(v);return *this;} void FieldNode::setOrientation(FieldOrientation v)noexcept{if(orientation_!=v){orientation_=v;markDirty(DirtyFlag::Layout);}} FieldOrientation FieldNode::orientation()const noexcept{return orientation_;}
FieldNode& FieldNode::enabled(bool v)noexcept{setEnabled(v);return *this;} void FieldNode::setEnabled(bool v)noexcept{if(enabled_==v)return;enabled_=v;if(auto* n=control())if(auto* c=dynamic_cast<ControlNode*>(n))c->setEnabled(v);markDirty(DirtyFlag::Paint);} bool FieldNode::isEnabled()const noexcept{return enabled_;}
FieldNode& FieldNode::control(std::unique_ptr<Node> v){setControl(std::move(v));return *this;} void FieldNode::setControl(std::unique_ptr<Node> v){clearChildren();if(v)appendChild(std::move(v));syncControlSemantics();markDirty(DirtyFlag::Layout);} Node* FieldNode::control()const noexcept{return children().empty()?nullptr:children().front().get();}
void FieldNode::syncControlSemantics(){if(auto* i=dynamic_cast<TextFieldNode*>(control())){if(!label_.empty())i->setAccessibleLabel(label_);i->setInvalid(validationState_==FieldValidationState::Error);i->setEnabled(enabled_);}else if(auto* c=dynamic_cast<ControlNode*>(control()))c->setEnabled(enabled_);} float FieldNode::labelWidth()const noexcept{return widthOf(label_+(required_?" *":""),theme().typography.body1Strong);}
SizeF FieldNode::measure(const Constraints& c)const{const auto&t=theme();auto* n=control();const SizeF cs=n?n->measureWithConstraints(c):SizeF{};const float lh=label_.empty()?0:t.typography.body1Strong.lineHeight,hh=hint_.empty()?0:t.typography.caption1.lineHeight,vh=validationMessage_.empty()?0:t.typography.caption1.lineHeight;if(orientation_==FieldOrientation::Horizontal){const float lw=label_.empty()?0:labelWidth()+kHGap;return c.clamp({lw+cs.width,std::max(lh,cs.height+(hh?kGap+hh:0)+(vh?kGap+vh:0))});}const float gaps=(lh&&cs.height?kGap:0)+(hh?kGap:0)+(vh?kGap:0);return c.clamp({std::max({labelWidth(),cs.width,widthOf(hint_,t.typography.caption1),widthOf(validationMessage_,t.typography.caption1)}),lh+cs.height+hh+vh+gaps});}
void FieldNode::layout(const RectF& b){Node::layout(b);if(auto*n=control()){const auto&t=theme();const float lw=orientation_==FieldOrientation::Horizontal&&!label_.empty()?std::min(b.width,labelWidth()+kHGap):0;const float y=orientation_==FieldOrientation::Vertical&&!label_.empty()?b.y+t.typography.body1Strong.lineHeight+kGap:b.y;const SizeF s=n->measureWithConstraints({0,std::max(0.f,b.width-lw),0,b.height});n->layout({b.x+lw,y,std::max(0.f,b.width-lw),s.height});}clearLayoutDirtyRecursively();}
void FieldNode::paint(PaintContext& c){const auto&t=theme();const auto&ls=t.typography.body1Strong;const auto&hs=t.typography.caption1;auto*n=control();const float lx=bounds().x,head=orientation_==FieldOrientation::Horizontal&&!label_.empty()?std::min(bounds().width,labelWidth()+kHGap):0;const Color fg=enabled_?t.colors.neutralForeground1:t.colors.neutralForegroundDisabled;if(!label_.empty()){RectF b{lx,bounds().y,head?head-kHGap:bounds().width,ls.lineHeight};c.drawText(label_,b.x,c.centeredTextBottom(label_,b,ls.size,ls.weight),ls.size,fg,ls.weight,ls.family);if(required_)c.drawText("*",b.x+widthOf(label_,ls)+t.spacing.horizontal.xs,c.centeredTextBottom("*",b,ls.size,ls.weight),ls.size,t.colors.statusDanger,ls.weight,ls.family);}float y=n?n->bounds().y+n->bounds().height:bounds().y;const float x=bounds().x+head,w=std::max(0.f,bounds().width-head);if(!hint_.empty()){y+=kGap;RectF b{x,y,w,hs.lineHeight};c.drawText(hint_,x,c.centeredTextBottom(hint_,b,hs.size,hs.weight),hs.size,enabled_?t.colors.neutralForeground3:t.colors.neutralForegroundDisabled,hs.weight,hs.family);y+=hs.lineHeight;}if(!validationMessage_.empty()){y+=kGap;RectF b{x,y,w,hs.lineHeight};c.drawText(validationMessage_,x,c.centeredTextBottom(validationMessage_,b,hs.size,hs.weight),hs.size,validation(validationState_,t),hs.weight,hs.family);}ContainerNode::paint(c);clearDirty(DirtyFlag::Paint);}

MessageBarNode::MessageBarNode(std::string v):body_(std::move(v)){} MessageBarNode& MessageBarNode::title(std::string v){setTitle(std::move(v));return *this;}void MessageBarNode::setTitle(std::string v){if(title_!=v){title_=std::move(v);markDirty(DirtyFlag::Layout);}}const std::string& MessageBarNode::title()const noexcept{return title_;}MessageBarNode& MessageBarNode::body(std::string v){setBody(std::move(v));return *this;}void MessageBarNode::setBody(std::string v){if(body_!=v){body_=std::move(v);markDirty(DirtyFlag::Layout);}}const std::string& MessageBarNode::body()const noexcept{return body_;}MessageBarNode& MessageBarNode::intent(MessageBarIntent v)noexcept{setIntent(v);return *this;}void MessageBarNode::setIntent(MessageBarIntent v)noexcept{if(intent_!=v){intent_=v;markDirty(DirtyFlag::Paint);}}MessageBarIntent MessageBarNode::intent()const noexcept{return intent_;}MessageBarNode& MessageBarNode::multiline(bool v)noexcept{setMultiline(v);return *this;}void MessageBarNode::setMultiline(bool v)noexcept{if(multiline_!=v){multiline_=v;markDirty(DirtyFlag::Layout);}}bool MessageBarNode::isMultiline()const noexcept{return multiline_;}
MessageBarNode& MessageBarNode::addAction(MessageBarAction v){actions_.push_back(std::move(v));markDirty(DirtyFlag::Layout);return *this;}MessageBarNode& MessageBarNode::clearActions(){actions_.clear();markDirty(DirtyFlag::Layout);return *this;}const std::vector<MessageBarAction>& MessageBarNode::actions()const noexcept{return actions_;}MessageBarNode& MessageBarNode::dismissible(bool v)noexcept{setDismissible(v);return *this;}void MessageBarNode::setDismissible(bool v)noexcept{if(dismissible_!=v){dismissible_=v;markDirty(DirtyFlag::Layout);}}bool MessageBarNode::isDismissible()const noexcept{return dismissible_;}MessageBarNode& MessageBarNode::onDismiss(DismissHandler h){onDismiss_=std::move(h);return *this;}
SizeF MessageBarNode::measure(const Constraints& c)const{const auto&t=theme();const float trailing=kPad+(dismissible_?kActionH+kMsgGap:0)+(multiline_?0.f:actions_.size()*kActionW);const float chrome=kPad+kIcon+kMsgGap+trailing;const float titleWidth=widthOf(title_,t.typography.body1Strong);const float bodyWidth=widthOf(body_,t.typography.body1);const float inlineGap=!title_.empty()&&!body_.empty()?4.f:0.f;const float available=std::max(1.f,c.maxWidth-chrome);const std::size_t lines=multiline_?std::max<std::size_t>(1,std::size_t(std::ceil((titleWidth+inlineGap+bodyWidth)/available))):1;const float actionRow=multiline_&&!actions_.empty()?kMsgGap+kActionH+kMsgGap:0.f;const float height=multiline_?10.f+lines*t.typography.body1.lineHeight+actionRow+8.f:std::max(36.f,t.typography.body1.lineHeight);return c.clamp({chrome+titleWidth+inlineGap+bodyWidth,height});}void MessageBarNode::layout(const RectF& b){Node::layout(b);clearLayoutDirtyRecursively();}
RectF MessageBarNode::dismissBounds()const noexcept{return {bounds().x+bounds().width-kPad-kActionH,bounds().y+kPad,kActionH,kActionH};}RectF MessageBarNode::actionBounds(std::size_t i)const noexcept{const float r=bounds().x+bounds().width-kPad-(dismissible_?kActionH+kMsgGap:0);return {r-kActionW*float(actions_.size()-i),bounds().y+bounds().height-kPad-kActionH,kActionW,kActionH};}
void MessageBarNode::paint(PaintContext& c)
{
    const auto& t = theme();
