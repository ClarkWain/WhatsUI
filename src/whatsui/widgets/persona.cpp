...
{
    // This is the Fluent v9 PersonaNode mapping: 20, 28, 32, 36, 40 and 56 DIP.
    switch (value) {
...
PersonaNode::PersonaNode(std::string name, PersonaSize size)
    : name_(std::move(name)), size_(size)
{
    syncChildren();
}

const std::string& PersonaNode::name() const noexcept { return name_; }
PersonaNode& PersonaNode::name(std::string value) { setName(std::move(value)); return *this; }
void PersonaNode::setName(std::string value)
{
...
const std::string& PersonaNode::primaryText() const noexcept { return primaryText_.empty() ? name_ : primaryText_; }
PersonaNode& PersonaNode::primaryText(std::string value) { setPrimaryText(std::move(value)); return *this; }
void PersonaNode::setPrimaryText(std::string value) { if (primaryText_ != value) { primaryText_ = std::move(value); markDirty(DirtyFlag::Layout); } }
const std::string& PersonaNode::secondaryText() const noexcept { return secondaryText_; }
PersonaNode& PersonaNode::secondaryText(std::string value) { setSecondaryText(std::move(value)); return *this; }
void PersonaNode::setSecondaryText(std::string value) { if (secondaryText_ != value) { secondaryText_ = std::move(value); markDirty(DirtyFlag::Layout); } }
const std::string& PersonaNode::tertiaryText() const noexcept { return tertiaryText_; }
PersonaNode& PersonaNode::tertiaryText(std::string value) { setTertiaryText(std::move(value)); return *this; }
void PersonaNode::setTertiaryText(std::string value) { if (tertiaryText_ != value) { tertiaryText_ = std::move(value); markDirty(DirtyFlag::Layout); } }
const std::string& PersonaNode::quaternaryText() const noexcept { return quaternaryText_; }
PersonaNode& PersonaNode::quaternaryText(std::string value) { setQuaternaryText(std::move(value)); return *this; }
void PersonaNode::setQuaternaryText(std::string value) { if (quaternaryText_ != value) { quaternaryText_ = std::move(value); markDirty(DirtyFlag::Layout); } }

PersonaNode& PersonaNode::avatarColor(AvatarColor value) noexcept { setAvatarColor(value); return *this; }
void PersonaNode::setAvatarColor(AvatarColor value) noexcept { if (avatarColor_ != value) { avatarColor_ = value; syncChildren(); markDirty(DirtyFlag::Paint); } }
AvatarColor PersonaNode::avatarColor() const noexcept { return avatarColor_; }
PersonaNode& PersonaNode::avatarShape(AvatarShape value) noexcept { setAvatarShape(value); return *this; }
void PersonaNode::setAvatarShape(AvatarShape value) noexcept { if (avatarShape_ != value) { avatarShape_ = value; syncChildren(); markDirty(DirtyFlag::Paint); } }
AvatarShape PersonaNode::avatarShape() const noexcept { return avatarShape_; }
PersonaNode& PersonaNode::avatarImage(ImageSource value) { setAvatarImage(std::move(value)); return *this; }
void PersonaNode::setAvatarImage(ImageSource value) { avatarImage_ = std::move(value); syncChildren(); markDirty(DirtyFlag::Paint); }
void PersonaNode::clearAvatarImage() noexcept { if (avatarImage_) { avatarImage_.reset(); syncChildren(); markDirty(DirtyFlag::Paint); } }

PersonaNode& PersonaNode::presence(PresenceStatus value) noexcept { setPresence(value); return *this; }
void PersonaNode::setPresence(PresenceStatus value) noexcept { if (presence_ != value) { presence_ = value; syncChildren(); markDirty(DirtyFlag::Layout); } }
void PersonaNode::clearPresence() noexcept { if (presence_) { presence_.reset(); syncChildren(); markDirty(DirtyFlag::Layout); } }
std::optional<PresenceStatus> PersonaNode::presence() const noexcept { return presence_; }
PersonaNode& PersonaNode::presenceOnly(bool value) noexcept { setPresenceOnly(value); return *this; }
void PersonaNode::setPresenceOnly(bool value) noexcept { if (presenceOnly_ != value) { presenceOnly_ = value; syncChildren(); markDirty(DirtyFlag::Layout); } }
bool PersonaNode::isPresenceOnly() const noexcept { return presenceOnly_; }

PersonaNode& PersonaNode::size(PersonaSize value) noexcept { setSize(value); return *this; }
void PersonaNode::setSize(PersonaSize value) noexcept { if (size_ != value) { size_ = value; syncChildren(); markDirty(DirtyFlag::Layout); } }
PersonaSize PersonaNode::size() const noexcept { return size_; }
PersonaNode& PersonaNode::textPosition(PersonaTextPosition value) noexcept { setTextPosition(value); return *this; }
void PersonaNode::setTextPosition(PersonaTextPosition value) noexcept { if (textPosition_ != value) { textPosition_ = value; markDirty(DirtyFlag::Layout); } }
PersonaTextPosition PersonaNode::textPosition() const noexcept { return textPosition_; }
PersonaNode& PersonaNode::textAlignment(PersonaTextAlignment value) noexcept { setTextAlignment(value); return *this; }
void PersonaNode::setTextAlignment(PersonaTextAlignment value) noexcept { if (textAlignment_ != value) { textAlignment_ = value; markDirty(DirtyFlag::Paint); } }
PersonaTextAlignment PersonaNode::textAlignment() const noexcept { return textAlignment_; }
PersonaNode& PersonaNode::accessibleLabel(std::string value) { setAccessibleLabel(std::move(value)); return *this; }
void PersonaNode::setAccessibleLabel(std::string value) { if (accessibleLabel_ != value) { accessibleLabel_ = std::move(value); markDirty(DirtyFlag::Style); } }
const std::string& PersonaNode::accessibleLabel() const noexcept { return accessibleLabel_; }
std::string PersonaNode::generatedAccessibleLabel() const
{
    ...
    if (presence_) { if (!result.empty()) result += ", "; result += PresenceBadgeNode(*presence_).generatedAccessibleLabel(); }
    return result;
}

PersonaNode& PersonaNode::onClick(ClickHandler handler) { onClick_ = std::move(handler); markDirty(DirtyFlag::Style); return *this; }
bool PersonaNode::isInteractive() const noexcept { return static_cast<bool>(onClick_); }
AvatarSize PersonaNode::avatarSize() const noexcept { return toAvatarSize(size_); }

void PersonaNode::syncChildren()
{
    clearChildren();
    if (presenceOnly_) {
        if (presence_) {
            auto badge = std::make_unique<PresenceBadgeNode>(*presence_);
            badge->setAvatarSize(avatarExtent(size_));
            appendChild(std::move(badge));
        }
        return;
    }
    auto avatar = std::make_unique<AvatarNode>(name_, avatarSize());
...
std::vector<std::string> PersonaNode::textLines() const
{
...
PersonaNode::TextLineStyle PersonaNode::textStyle(std::size_t index) const noexcept
{
    const auto& typography = theme().typography;
    if (index == 0) {
        // Fluent PersonaNode uses body1 by default and upgrades the primary line
...
float PersonaNode::textWidth(const std::string& text, const TextLineStyle& style) const noexcept
{
...
std::string PersonaNode::ellipsize(const std::string& text, float width, const TextLineStyle& style) const
{
...
float PersonaNode::textBlockHeight() const noexcept
{
...
float PersonaNode::preferredTextWidth() const noexcept
{
    const auto lines = textLines();
    float width = 0.0f;
...
float PersonaNode::mediaSpacing() const noexcept
{
    // Fluent PersonaNode's media spacing uses the nudge scale, rather than one
...
SizeF PersonaNode::measure(const Constraints& constraints) const
{
...
void PersonaNode::layout(const RectF& bounds)
{
    Node::layout(bounds);
    const float gap = children().empty() ? 0.0f : mediaSpacing();
    const float indicator = children().empty() ? 0.0f : (presenceOnly_ ? children().front()->measureWithConstraints({}).width : avatarExtent(size_));
    ...
    if (!children().empty()) {
        ...
        if (!presenceOnly_ && children().size() > 1) {
            auto* badge = static_cast<PresenceBadgeNode*>(children()[1].get());
...
void PersonaNode::paint(PaintContext& context)
{
...
void PersonaNode::invoke() { if (onClick_) onClick_(); }
bool PersonaNode::onPointerEvent(const PointerEvent& event)
{
...
bool PersonaNode::onKeyEvent(const KeyEvent& event)
{
    if (!isInteractive() || !isEnabled() || event.action != KeyAction::Down || (event.keyCode != 13 && event.keyCode != 32)) return false;
    invoke();
    return true;
}
AccessibilityActionCapabilities PersonaNode::accessibilityActions() const noexcept { AccessibilityActionCapabilities actions; actions.invoke = isInteractive(); return actions; }
AccessibilityActionStatus PersonaNode::performAccessibilityAction(AccessibilityActionKind kind, std::string_view value)
{
    (void)value;
