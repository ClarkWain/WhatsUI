...
AccessibilityProperties propertiesForNode(const Node& node, const Node* focused)
{
    ...
    if (const auto* popoverButton = dynamic_cast<const PopoverButtonNode*>(&node)) {
    ...
    } else if (const auto* persona = dynamic_cast<const PersonaNode*>(&node)) {
    ...
    } else if (const auto* dataGrid = dynamic_cast<const DataGridNode*>(&node)) {
    ...
    } else if (const auto* table = dynamic_cast<const TableNode*>(&node)) {
        properties.role = AccessibilityRole::Table;
        properties.label = table->accessibleLabel();
    } else if (const auto* calendar = dynamic_cast<const CalendarNode*>(&node)) {
        properties.role = AccessibilityRole::Calendar;
        properties.label = "CalendarNode";
        properties.value = formatIsoDate(calendar->focusedDate());
    } else if (const auto* datePicker = dynamic_cast<const DatePickerNode*>(&node)) {
    ...
    } else if (const auto* timePicker = dynamic_cast<const TimePickerNode*>(&node)) {
        properties.role = AccessibilityRole::ComboBox;
        properties.label = "Time";
        properties.value = timePicker->text();
        properties.expanded = timePicker->isOpen();
    } else if (const auto* tree = dynamic_cast<const TreeNode*>(&node)) {
        properties.role = AccessibilityRole::Tree;
        properties.label = tree->accessibleLabel();
    } else if (const auto* treeItem = dynamic_cast<const TreeItemNode*>(&node)) {
    ...
    } else if (const auto* teachingPopover = dynamic_cast<const TeachingPopoverNode*>(&node)) {
        properties.role = AccessibilityRole::Dialog;
        properties.label = teachingPopover->title();
        properties.description = teachingPopover->body();
    } else if (const auto* popover = dynamic_cast<const PopoverNode*>(&node)) {
        properties.role = AccessibilityRole::Group;
        properties.label = popover->accessibleLabel().empty() ? popover->title() : popover->accessibleLabel();
        properties.description = popover->body();
    } else if (const auto* menuButton = dynamic_cast<const MenuButtonNode*>(&node)) {
        properties.role = AccessibilityRole::Button;
        properties.label = menuButton->label();
        properties.expanded = menuButton->isOpen();
    } else if (const auto* splitButton = dynamic_cast<const SplitButtonNode*>(&node)) {
        properties.role = AccessibilityRole::Button;
        properties.label = splitButton->label();
        properties.expanded = splitButton->isOpen();
    } else if (const auto* compoundButton = dynamic_cast<const CompoundButtonNode*>(&node)) {
        properties.role = AccessibilityRole::Button;
        properties.label = compoundButton->label();
        properties.description = compoundButton->secondaryContent();
    } else if (const auto* toggleButton = dynamic_cast<const ToggleButtonNode*>(&node)) {
        properties.role = AccessibilityRole::Button;
        properties.label = toggleButton->label();
        properties.checked = toggleButton->isChecked();
    } else if (const auto* button = dynamic_cast<const ButtonNode*>(&node)) {
        properties.role = AccessibilityRole::Button;
        properties.label = button->label();
    } else if (const auto* iconButton = dynamic_cast<const IconButtonNode*>(&node)) {
    ...
    } else if (const auto* checkbox = dynamic_cast<const CheckboxNode*>(&node)) {
    ...
    } else if (const auto* group = dynamic_cast<const RadioGroupNode*>(&node)) {
    ...
    } else if (const auto* radio = dynamic_cast<const RadioNode*>(&node)) {
        properties.role = AccessibilityRole::RadioButton;
        properties.label = radio->label();
        properties.checked = radio->isSelected();
    } else if (const auto* toggle = dynamic_cast<const SwitchNode*>(&node)) {
    ...
    } else if (const auto* rating = dynamic_cast<const RatingNode*>(&node)) {
        properties.role = AccessibilityRole::RadioGroup;
    ...
    } else if (const auto* slider = dynamic_cast<const SliderNode*>(&node)) {
    ...
    } else if (const auto* progress = dynamic_cast<const ProgressBarNode*>(&node)) {
    ...
    } else if (const auto* dropdown = dynamic_cast<const DropdownNode*>(&node)) {
        properties.role = AccessibilityRole::ComboBox;
    ...
    } else if (const auto* listBox = dynamic_cast<const ListBoxNode*>(&node)) {
    ...
    } else if (const auto* input = dynamic_cast<const TextFieldNode*>(&node)) {
    ...
    } else if (const auto* link = dynamic_cast<const LinkNode*>(&node)) {
        properties.role = AccessibilityRole::Link;
        properties.label = link->label();
        properties.description = link->href();
    } else if (const auto* breadcrumb = dynamic_cast<const BreadcrumbNode*>(&node)) {
        properties.role = AccessibilityRole::Group;
        properties.label = breadcrumb->accessibleLabel();
    } else if (const auto* breadcrumbItem = dynamic_cast<const BreadcrumbItemNode*>(&node)) {
    ...
    } else if (const auto* badge = dynamic_cast<const BadgeNode*>(&node)) {
        properties.role = AccessibilityRole::Text;
        properties.label = badge->generatedAccessibleLabel();
    } else if (const auto* counter = dynamic_cast<const CounterBadgeNode*>(&node)) {
        const std::string label = counter->generatedAccessibleLabel();
        if (label.empty()) return properties;
        properties.role = AccessibilityRole::Text;
        properties.label = label;
        properties.value = counter->text();
    } else if (const auto* presence = dynamic_cast<const PresenceBadgeNode*>(&node)) {
        properties.role = AccessibilityRole::Text;
        properties.label = presence->generatedAccessibleLabel();
    } else if (const auto* card = dynamic_cast<const CardNode*>(&node)) {
        // A passive CardNode is grouping content; a selectable CardNode is exposed as
    ...
    } else if (dynamic_cast<const MenuNode*>(&node) != nullptr) {
        properties.role = AccessibilityRole::Menu;
    } else if (dynamic_cast<const DialogNode*>(&node) != nullptr) {
        properties.role = AccessibilityRole::Dialog;
    } else if (const auto* ratingDisplay = dynamic_cast<const RatingDisplayNode*>(&node)) {
        properties.role = AccessibilityRole::Image;
        properties.label = ratingDisplay->generatedAccessibleLabel();
    } else if (const auto* image = dynamic_cast<const ImageNode*>(&node)) {
        if (image->isDecorative()) return properties;
        properties.role = AccessibilityRole::Image;
        properties.label = image->alt();
    } else if (const auto* divider = dynamic_cast<const DividerNode*>(&node)) {
        properties.role = AccessibilityRole::Separator;
        properties.label = divider->content();
    } else if (const auto* field = dynamic_cast<const FieldNode*>(&node)) {
        properties.role = AccessibilityRole::Group;
    ...
    } else if (const auto* message = dynamic_cast<const MessageBarNode*>(&node)) {
        properties.role = AccessibilityRole::Alert;
        ...
        properties.live = true;
    } else if (const auto* container = dynamic_cast<const BoxNode*>(&node);
               container != nullptr && container->interaction() != nullptr) {
        // BoxNode with an attached InteractionArea projects the role and
        // label the caller declared (e.g. wui::Box().accessibleRole(...).
    ...
    if (dynamic_cast<const DrawerNode*>(&node) != nullptr) {
        properties.actions.focus = true;
    }
    ...
    // TableNode deliberately models a passive, read-only data presentation.
    ...
    // detail. DataGridNode keeps the normal interactive control focus contract.
    if (dynamic_cast<const TableNode*>(&node) != nullptr &&
        dynamic_cast<const DataGridNode*>(&node) == nullptr) {
        properties.actions.focus = false;
    }
    // Passive Personas are presentational identity groups, not focus stops.
    // Interactive Personas opt in to ButtonNode semantics through onClick().
...
{
    // A linked inactive TabPanelNode is intentionally absent from both the visual
    ...
    if (const auto* tabPanel = dynamic_cast<const TabPanelNode*>(&node);
    ...
    // AccordionNode items retain their body nodes across collapse/expand cycles,
    // but a collapsed body is not part of the active semantic surface.
    // Preserve the item ButtonNode itself while preventing hidden descendants from
    // being exposed to a screen reader or Windows UI Automation.
    if (const auto* accordionItem = dynamic_cast<const AccordionItemNode*>(&node);
        accordionItem != nullptr && !accordionItem->isExpanded()) {
        return;
    }
    // TreeNode items retain descendants across collapse/expand cycles. Keep the
    // current TreeItemNode in the semantic tree, but never expose its hidden
    // branch to UI Automation or a screen reader.
    if (const auto* treeItem = dynamic_cast<const TreeItemNode*>(&node);
        treeItem != nullptr && !treeItem->isExpanded()) {
        return;
    }

    // A ListBoxNode windows its visual rows, but its visible options still need
    ...
    // back to the owning ListBoxNode by option value.
    if (const auto* listBox = dynamic_cast<const ListBoxNode*>(&node)) {
    ...
    // Tables window their rows just like ListBoxNode. Materialize only the header
    ...
    if (const auto* table = dynamic_cast<const TableNode*>(&node)) {
        const bool dataGrid = dynamic_cast<const DataGridNode*>(table) != nullptr;
